...
--[[ local function GryllsUnitFrames_tick()
    if GryllsUnitFrames_Settings.tick then
        if not Grylls_Tick then
            ...
            if ( Grylls_Tick.class == "Rogue" ) then
                Grylls_Tick.tickRate = 2 -- tick once every two seconds
            else
                Grylls_Tick.tickRate = 5 -- tick once every five seconds (MP5) -- TODO
            end

            GryllsUnitFrames_tick_SetScript(true)
        end
    end
end

local function GryllsUnitFrames_tick_SetScript(bool)
    if bool == true then
        if ( Grylls_Tick.class == "Rogue" or Grylls_Tick.class == "Druid" ) then --- enable for energy
            Grylls_Tick:SetScript("OnUpdate", function()
...
local function GryllsUnitFrames_commands(msg, editbox)
    ...
    elseif msg == "tick" then
        if GryllsUnitFrames_Settings.tick then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("GryllsUnitFrames: tick hidden")
            GryllsUnitFrames_tick_SetScript(false)
        else
            GryllsUnitFrames_Settings.tick = true
    ...
    elseif msg == "color health" then
        if GryllsUnitFrames_Settings.colorhealth then
            ...
            GryllsUnitFrames_Settings.colorhealth = true
            DEFAULT_CHAT_FRAME:AddMessage("GryllsUnitFrames: health coloring on (requires reload)")
            GryllsUnitFrames_updateUnitFrames()
        end
    ...
    elseif msg == "color power" then
        if GryllsUnitFrames_Settings.colorpower then
            ...
            DEFAULT_CHAT_FRAME:AddMessage("GryllsUnitFrames: power coloring off (requires reload)")
            GryllsUnitFrames_updateUnitFrames()
        else
            GryllsUnitFrames_Settings.colorpower = true
            DEFAULT_CHAT_FRAME:AddMessage("GryllsUnitFrames: power coloring on (requires reload)")
            GryllsUnitFrames_updateUnitFrames()
        end
